  ...
  <v-card>
      ...
      <v-form ref="form" @submit="save">
        <p>
          <span class="font-weight-bold">
            {{
              $t(
                "modules.userCloudManager.components.shareFile.template.currentPathTitle"
              )
            }}
          </span>
          <span>{{ currentFile.fullDirectoryPath }}</span>
        </p>
        <v-select
          ...
          :label="
            $t(
              'modules.userCloudManager.components.shareFile.template.sharedUserGroupsTitle'
            )
          "
          multiple
          item-text="groupName"
        ...
        <v-select
          ...
          :label="
            $t(
              'modules.userCloudManager.components.shareFile.template.sharedUsersTitle'
            )
          "
          multiple
          item-text="fullName"
    ...
    <v-card-actions>
      ...
      <v-btn color="danger" text @click="closeDialog">
        {{
          $t("modules.userCloudManager.components.shareFile.template.cancelBtn")
        }}
      </v-btn>
      <v-btn color="primary" text @click="save">
        {{
          $t("modules.userCloudManager.components.shareFile.template.saveBtn")
        }}
      </v-btn>
    </v-card-actions>
  </v-card>
  ...
  methods: {
    ...
    clearForm() {
      setTimeout(() => {
        ...
        if (this.currentFile.shared.groups.length > 0) {
          this.currentFile.shared.groups.forEach(item => {
            this.inputs.selectedGroups.push(item.sharedGroupId);
          });
        }

        if (this.currentFile.shared.users.length > 0) {
          this.currentFile.shared.users.forEach(item => {
    ...
    save() {
      this.$store.commit(
        ...
        t.t(
          "modules.userCloudManager.components.shareFile.script.methods.save.loading"
        )
      );

      let sendData = {
        fileId: this.currentFile.value,
      ...
      this.$store
        .dispatch(
          "a_storages_modules_userCloudManager_fm_shareFile_api",
          sendData
        )
        .then(() => {
          Tools.showSuccessMsg(
            t.t(
              "modules.userCloudManager.components.shareFile.script.methods.save.successTitle"
            ),
            t.t(
              "modules.userCloudManager.components.shareFile.script.methods.save.successMsg"
            )
          );
